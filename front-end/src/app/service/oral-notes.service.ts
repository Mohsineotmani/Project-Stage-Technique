...
import {ErrorMessageService} from "./Error/error-message.service"; // Import the TokenService

@Injectable({
  providedIn: 'root'
})
export class OralNotesService {
  private apiUrl = 'http://localhost:8082/api/oral-notes';
  ...
  publishResults(): Observable<any> {
    ...
    return this.http.post<any>('http://localhost:8082/api/oral-notes/publish', {}, {headers});
  }
  ...
  publishOralResults(): Observable<any> {
    ...
    return this.http.post<any>('http://localhost:8082/api/oral-notes/publishOralResults', {}, {headers}).pipe(
