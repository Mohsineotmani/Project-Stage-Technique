...
export class NotesComponent implements OnInit {
  ...
  fetchNotes(): void {
    this.notesService.getAllNotes().subscribe((data: any[]) => {
  ...
  updateSeuil(): void {
    ...
    this.http.put('http://localhost:8082/Passerelle/seuil_orale', {seuil: this.seuilOral}).subscribe(
      () => {
        alert('Seuil oral mis à jour avec succès.');
  ...
  downloadCsv(): void {
    this.notesService.downloadCsv().subscribe((blob) => {
  ...
  sendEmailsToAdmitted(): void {
    ...
    if (emails.length > 0) {
      this.notesService.sendEmailsToAdmitted(this.seuilOral, emails).subscribe({
  ...
  publishResults(): void {
    ...
    this.notesService.publishResults().subscribe({
      next: () => {
        console.log('Les résultats ont été publiés avec succès.');
        this.errorMessageService.showSuccessMessage('Les résultats ont été publiés avec succès.',"Publié")
        this.fetchNotes();
      },
      error: (error) => {
        if (error.status === 409) {
          this.errorMessageService.showErrorMessage(error.error?.message || "Error notesService publishResults 409 ." ,"Erreur de publication :");
        } else {
          this.errorMessageService.showErrorMessage(error.error?.message || "Erreur lors de la publication des résultats." ,"Erreur de publication :");
  ...
  publishOralResults(): void {
    ...
    this.notesService.publishOralResults().subscribe({
      next: (response) => {
        console.log(response.message);
        alert('Les résultats de l’oral ont été publiés avec succès.');
      },
      error: (error) => {
  ...
  cancelPublishResults(): void {
    this.http.post<any>('http://localhost:8082/api/notes/cancelPublish', {})
      .subscribe(
        response => {
          console.log('Réponse:', response);
          this.errorMessageService.showSuccessMessage(response.message ,"Note Annulée");
...
// Annuler la publication des résultats finaux
  cancelPublishFinalResults(): void {
    this.http.post<any>('http://localhost:8082/api/notes/cancelPublishFinalResults', {})
      .subscribe(
        response => {
          console.log('Réponse:', response);
