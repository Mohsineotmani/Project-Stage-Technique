...
@Component({
  selector: 'app-oral-notes',
  templateUrl: './oral-notes.component.html',
  styleUrls: ['./oral-notes.component.scss'],
})
export class OralNotesComponent implements OnInit {
  ...
  constructor(
    private oralNotesService: OralNotesService,
  ...
  fetchNotes(): void {
    this.oralNotesService.getAllNotes().subscribe((data: any[]) => {
  ...
  updateSeuil(): void {
    if (this.seuilOral <= 0) {
      this.errorMessageService.showErrorMessage("Veuillez entrer un seuil oral valide." , "Seuil valide");
      return;
    }

    this.http.put('http://localhost:8082/Passerelle/seuil_orale', {seuil: this.seuilOral}).subscribe(
      () => {
        this.errorMessageService.showSuccessMessage("Seuil oral mis à jour avec succès." , "Seuil ");
      ...
      (error) => {
        ...
        this.errorMessageService.showErrorMessage("Erreur lors de la update  du seuil oral." , "Seuil valide");
      }
    );
  ...
  generatePdf(): void {
    ...
    if (admitCandidates.length === 0) {
      const  varAffiche = this.selectedFiliere
      ...
      this.errorMessageService.showErrorMessage(varAffiche , "Seuil valide");
      return;
    }
    ...
    img.onerror = () => {
      this.errorMessageService.showErrorMessage("Impossible de charger le logo. Vérifiez l’URL" , "Logo valide");
    };
  }
  ...
  generateFinalResultsPdf(): void {
    ...
    if (finalResults.length === 0) {
      this.errorMessageService.showErrorMessage("Aucun candidat n'a le statut 'admit' pour l'oral." , "Donnée");
      return;
    }
    ...
    img.onload = () => {
      ...
      if (newWindow) {
        ...
        this.errorMessageService.showErrorMessage("Impossible d'ouvrir le PDF dans un nouvel onglet." , "PDF invalid");
      }
    };

    img.onerror = () => {
      console.error("Erreur lors du chargement du logo.");
      this.errorMessageService.showErrorMessage("Erreur lors de l'ajout du logo au PDF" , "Logo invalide");
  ...
  addNote(): void {
    this.router.navigate(['/oral-form']);
  }

  editNote(id: number): void {
    this.router.navigate(['/oral-form', id]);
  }

  deleteNote(id: number): void {
    this.oralNotesService.deleteNoteById(id).subscribe({
      next: data => {
        this.fetchNotes();
      }, error: err => {
  ...
  downloadCsv(): void {
    this.oralNotesService.downloadCsv().subscribe((blob) => {
  ...
  sendEmailsToAdmitted(): void {
    ...
    if (emails.length > 0) {
      this.oralNotesService.sendEmailsToAdmitted(this.seuilOral, emails).subscribe({
  ...
  publishResults(): void {
    ...
    this.oralNotesService.publishResults().subscribe({
      next: () => {
        console.log('Les résultats ont été publiés avec succès.');
        alert('Les résultats ont été publiés avec succès.');
        this.fetchNotes();
      },
      error: (error) => {
        if (error.status === 409) {
          this.errorMessageService.showErrorMessage(error.error?.message || "Error oralNotesService publishResults 409 ." ,"Erreur de publication :");
        } else {
          this.errorMessageService.showErrorMessage(error.error?.message || "Erreur lors de la publication des résultats Oral." ,"Erreur de publication :");
  ...
  publishOralResults(): void {
    ...
    this.oralNotesService.publishOralResults().subscribe({
      next: (response) => {
  ...
  cancelPublishResults(): void {
    this.http.post<any>('http://localhost:8082/api/oral-notes/cancelPublish', {})
      .subscribe(
        response => {
...
// Annuler la publication des résultats finaux
  cancelPublishFinalResults(): void {
    this.http.post<any>('http://localhost:8082/api/oral-notes/cancelPublishFinalResults', {})
      .subscribe(
        response => {
          console.log('Réponse:', response);
                    this.errorMessageService.showSuccessMessage(response.message, "Cancelled");
